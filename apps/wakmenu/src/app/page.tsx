...
export default function WakmenuPage() {
  ...
  const reveal = () => { const audio = completeAudio.current; if (audio) { void audio.play().then(() => { audio.pause(); audio.currentTime = 0 }).catch(() => undefined) } store.engine.reveal() }
  return <main className="page-shell"><audio ref={completeAudio} src={`${BASE}/complete.mp3`} preload="auto"/><header><div><p className="eyebrow">WAKGOOD’S LUNCH GAME</p><h1>우왁굳의 <em>밥</em>을 맞춰라!</h1><p className="subtitle">시청자는 <code>!밥 메뉴명</code>으로 정답을 외쳐주세요.</p></div><div className="wakdu-frame"><img className="wakdu" src={`${BASE}/wakdu.gif`} alt="밥 먹는 우왁굳" /></div></header>
    ...
    <section className="game-board"><div className="phase-line"><span className={`phase ${snapshot.phase}`}>{phaseText[snapshot.phase]}</span><span className="phase-actions">{remaining != null && <strong className="timer">{formatTime(remaining)}</strong>}{snapshot.phase === 'revealed' && <button className="new-game" type="button" onClick={() => store.engine.reset()}><RotateCcw size={14}/> 새 게임</button>}</span></div>
      ...
      {snapshot.phase === 'closed' && <div className="live-stage"><img className="small-wakdu" src={`${BASE}/wakdu.gif`} alt=""/><h2>쩝쩝… 정답을 공개할까요?</h2><button className="primary" type="button" onClick={reveal}><Sparkles size={17}/> 정답 공개</button></div>}
